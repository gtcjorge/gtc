...
function sfsubject(co) {
    ...
    c[16] = co.Instructor;
    //class info
    var classname, cityname, description, halforfull, time, type;
    type = "seminar";
    //class info
    ...
    var classdate = Date.parse(c[5]);
    var today = new Date();
    var datefield = classdate.toString('MM/dd/yyyy');
    var registrationfield = today.toString('MM/dd/yyyy');
    var testdate = (1).months().fromNow();
    var price = -1;
    ...
    var hoursfield = fTime(halforfull, type);
...
function go() {
    var id = $("#who_id").attr("value");

    // $("#tsk6").val(GM_getValue("token"));


    var urlr = 'https://na8.salesforce.com/services/data/v38.0/query/?q=SELECT Name from pymt__Shopping_Cart_Item__c WHERE pymt__Contact__c = \'' + id + '\''+' AND CreatedDate = LAST_N_DAYS:5';
    var urlcontactquery = 'https://na8.salesforce.com/services/data/v38.0/query/?q=SELECT Order_Source__c from Contact WHERE Id = \'' + id + '\'';
        ...
        onload: function(response) {
            ...
            if (response.status === 200) {
                ...
                if (json && json.totalSize === 1) {
                    var source = json.records[0].Order_Source__c;
                    // console.log(source);
                    var tsource = source;
            ...
            if (response.status === 200) {
                ...
                $("#classes").on('change', function() {
                    if (!$(this).find(":selected").val() || $(this).find(":selected").val() === "")
                        return;
                    var classselected = dict[$(this).find(":selected").val().split(" - ")[0]] + " - " + $(this).find(":selected").val().split(" - ")[1];
                            ...
                            if (response.status === 200) {
                                ...
                                if (json.length > 1) {
                                    ...
                                    $("#classesfound").on('change', function() {
                                        if (!$(this).find(":selected").val() || $(this).find(":selected").val() === "")
                                            return;
                                        var idfound = $(this).find(":selected").attr("id");
                ...
                console.error(response.responseText);
                var needsession = false;
                var ec = JSON.parse(response.responseText)[0].errorCode;
                console.log(ec);
    ...
    onload: function(response) {
        if (response.status === 200) {
            ...
            $("#tasksfromotherid").on("change", function() {
                ...
                var id = $("#tasksfromotherid option:selected").attr("id");
                GM_xmlhttpRequest({
                    method: "GET",
                    headers: {
                        'Authorization': 'OAuth ' + GM_getValue("token"),
